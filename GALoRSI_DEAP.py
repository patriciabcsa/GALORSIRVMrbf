...
from sklearn.metrics import accuracy_score, roc_auc_score,mean_absolute_error,accuracy_score,r2_score,confusion_matrix ,classification_report 
from sklearn.model_selection import train_test_split,StratifiedKFold,GridSearchCV,cross_val_score,cross_val_predict,cross_validate
...
dfData = pd.read_csv('MI_GA_/BD/BD_DEAP/pruebaDEAP.csv')
#/Users/patriciabecerra/Desktop/PhD./2019/Proyect/MI_GA_/BD/BD_DEAP/pruebaDEAP.csv

# Encode labels to numbers (in cases)
# Get classes and one hot encoded feature vectors
le = LabelEncoder()
le.fit(dfData['valence'])
allClasses = le.transform(dfData['valence'])#Label
allFeatures = dfData.drop(['user','video','op','valence','arousal'], axis=1)#Data
allFeatures = allFeatures.sample(n=15,axis='columns') # Validation Stage Chapter VI
...
def getHof():
	...
	numPop =5#100#100#50 #30 #100
	numGen =10#30#80#100#10
